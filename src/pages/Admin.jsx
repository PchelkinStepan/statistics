...
const Admin = () => {
  ...
  const handleEditMatch = (match) => {
    ...
    setActiveTab('match'); setShowMobileForm(true); window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  const handleIntChange = (field, value) => {
    ...
    if (field.toLowerCase().includes('possession') && val !== '') {
      const num = parseInt(val); if (num > 100) setMatchForm({...matchForm, [field]: '100'});
      else setMatchForm({...matchForm, [field]: val});
    } else setMatchForm({...matchForm, [field]: val});
  };
  ...
  const FieldRow = ({ label, fieldH, fieldA, isFloat }) => (
    <div className="grid grid-cols-2 gap-2">
      <div>
        <label className="block text-[11px] text-gray-400 mb-0.5">{label} Х</label>
        <input type="text" inputMode={isFloat ? 'decimal' : 'numeric'} value={matchForm[fieldH]} onFocus={(e) => e.target.select()}
          onChange={(e) => isFloat ? handleFloatChange(fieldH, e.target.value) : handleIntChange(fieldH, e.target.value)}
      ...
      <div>
        ...
        <input type="text" inputMode={isFloat ? 'decimal' : 'numeric'} value={matchForm[fieldA]} onFocus={(e) => e.target.select()}
          onChange={(e) => isFloat ? handleFloatChange(fieldA, e.target.value) : handleIntChange(fieldA, e.target.value)}
          className="w-full bg-gray-900 border border-gray-700 rounded-lg px-2 py-1.5 text-xs" />
      </div>
    </div>
  );
  ...
  const renderFields = (suffix) => {
    ...
    return (
      <div className="space-y-2">
        <FieldRow label="1. Голы" fieldH={`homeScore${s}`} fieldA={`awayScore${s}`} />
        <FieldRow label="2. Угловые" fieldH={`homeCorners${s}`} fieldA={`awayCorners${s}`} />
        <FieldRow label="3. Ожидаемые голы (xG)" fieldH={`homeXG${s}`} fieldA={`awayXG${s}`} isFloat />
        <FieldRow label="4. Владение (%)" fieldH={`homePossession${s}`} fieldA={`awayPossession${s}`} />
        <FieldRow label="5. Всего ударов" fieldH={`homeTotalShots${s}`} fieldA={`awayTotalShots${s}`} />
        <FieldRow label="6. Удары в створ" fieldH={`homeShotsOnTarget${s}`} fieldA={`awayShotsOnTarget${s}`} />
        <FieldRow label="7. Жёлтые карточки" fieldH={`homeYellowCards${s}`} fieldA={`awayYellowCards${s}`} />
        <FieldRow label="8. Красные карточки" fieldH={`homeRedCards${s}`} fieldA={`awayRedCards${s}`} />
        <FieldRow label="9. Удары из штрафной" fieldH={`homeShotsInsideBox${s}`} fieldA={`awayShotsInsideBox${s}`} />
        <FieldRow label="10. Сейвы" fieldH={`homeSaves${s}`} fieldA={`awaySaves${s}`} />
        <FieldRow label="11. Фолы" fieldH={`homeFouls${s}`} fieldA={`awayFouls${s}`} />
      </div>
    );
  };
