...
import { Save, Trash2, ChevronDown, ChevronUp, Plus, X, Edit, Check } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl">
      <div className="mb-4 md:mb-8">
        ...
        <p className="text-sm md:text-base text-gray-400">Расширенные метрики + синхронизация ☁️</p>
      </div>
      ...
      {isMobile && activeTab === 'match' && (
        <button
          ...
          className="fixed bottom-6 right-6 z-20 bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-full shadow-lg flex items-center justify-center"
        >
          <Plus size={28} />
        </button>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        {/* Форма (на десктопе всегда видна, на мобилке - модалка) */}
        {(!isMobile || showMobileForm) && (
          <div className={`${isMobile ? 'fixed inset-0 z-50 bg-gray-900 overflow-auto p-4' : 'lg:col-span-2'}`}>
            ...
            {activeTab === 'match' && (
              <form onSubmit={handleMatchSubmit} className="bg-gray-800 rounded-xl p-4 md:p-6 border border-gray-700 space-y-4 md:space-y-6">
                <div className="space-y-3">
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1">Лига</label>
                    <select value={matchForm.leagueId} onChange={(e) => setMatchForm({...matchForm, leagueId: e.target.value, homeTeamId: '', awayTeamId: ''})}
                      className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2.5 text-sm" required>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1">Хозяева</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1">Гости</label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs text-gray-400 mb-1">Угловые Х</label>
                    ...
                    <div>
                      <label className="block text-xs text-gray-400 mb-1">Угловые Г</label>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between px-3 py-2.5 bg-gray-700/50 rounded-lg text-sm"
                >
                ...
                <button type="submit" className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold py-3 rounded-lg text-sm flex items-center justify-center gap-2">
                  <Save size={18} />
                  Добавить матч
                </button>
            ...
            {activeTab === 'league' && (
              <form onSubmit={handleLeagueSubmit} className="bg-gray-800 rounded-xl p-4 md:p-6 border border-gray-700 space-y-4">
                <h3 className="text-lg font-bold mb-2">Добавить лигу</h3>
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Страна</label>
            ...
            {activeTab === 'team' && (
              <form onSubmit={handleTeamSubmit} className="bg-gray-800 rounded-xl p-4 md:p-6 border border-gray-700 space-y-4">
                <h3 className="text-lg font-bold mb-2">Добавить команду</h3>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Название</label>
